...
async function collect2020BettingLines() {
  console.log('🎯 Direct 2020 Betting Lines Collection');
  
  try {
    // Get 2020 betting lines from CFBD
    const response = await fetch(
      'https://api.collegefootballdata.com/lines?year=2020',
      {
    ...
    for (const line of bettingData) {
      ...
      if (processedLines < 3) {
        console.log('Sample betting data:', JSON.stringify(line, null, 2));
      }
      
      try {
        // Find the game in our database by matching teams and date
        const gameQuery = `
          SELECT g.id 
          FROM games g
